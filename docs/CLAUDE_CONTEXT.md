...
`SITE` dict — global site metadata (title, tagline, GitHub URL).
...
```bash
# 1. Author / edit a lesson in LESSONS/
# 2. Regenerate HTML lessons
python scripts/convert_md2html.py ./LESSONS -o ./HTML_LESSONS \
    --lang ar --prose --home-url ../index.html

# 3. Regenerate homepage (if sections/lessons changed)
python scripts/generate_homepage.py

# 4. Commit and push — GitHub Pages redeploys automatically
git add .
git commit -m "add lesson X"
git push
```
